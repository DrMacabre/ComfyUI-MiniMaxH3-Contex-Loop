...
# field. Seed-only Reroll must source the already-authored exact frame count.
...
exact_pos = final.index('? exactResponseLength(node, submittedReview, submittedReview)', reroll_pos)
if not (retry_pos < field_pos < reroll_pos < exact_pos):
    raise SystemExit("Review Reroll seed exact-frame branch ordering is wrong")

print("PASS Review Reroll seed is seed-only and preserves exact final frames")
